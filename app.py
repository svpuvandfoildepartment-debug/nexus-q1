...
if st.session_state.auto_sim_running:
    ...
    if _max > 0 and _done >= _max:
        ...
        push_alert(
            f"Auto #{st.session_state.sim_count} [{_strat[:16]}]: "
...
with tab_sim:
    ...
    if st.session_state.auto_sim_running:
        ...
        <div style='background:#eff6ff;border:2px solid #2563eb;border-radius:10px;
          ...
          <span style='font-size:0.75rem;color:#64748b;'>Strategy: {st.session_state.auto_sim_strat[:28]}</span>
        </div>
        """, unsafe_allow_html=True)
            ...
            if not running:
                if st.button("▶ RUN ONCE", use_container_width=True, key="btn_run_once"):
                    ...
                    for t in res["trade_log"]:
                        if t["type"]=="SELL": st.session_state.trade_journal.insert(0,{"time":datetime.now().strftime("%H:%M:%S"),"source":f"SIM#{st.session_state.sim_count}","strategy":sim_strat,"side":t["type"],"price":round(t["price"],4),"pnl":t["pnl"],"reason":t["reason"][:40]})
                    push_alert(f"Sim #{st.session_state.sim_count} [{sim_strat[:16]}]: {res['total_ret']:+.2f}% · WR {res['win_rate']:.0f}%","success" if res["total_ret"]>0 else "warning")
                    st.rerun()
            else:
