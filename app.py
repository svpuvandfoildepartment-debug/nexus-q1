...
DEFAULTS = {
    ...
    "auto_sim_strat":   "RSI Oversold/Overbought",
    "auto_sim_coin":    "BTC/USDT",
    "auto_sim_cap":     10000,
    "auto_sim_candles": 250,
    "auto_sim_pos":     90,
    "auto_sim_fee":     0.08,
    "auto_sim_sl":      4.0,
    "auto_sim_tp":      8.0,
...
def simulate_strategy(strategy,capital,candle_count,pos_size_pct,fee_pct,sl_pct,tp_pct,params,coin="BTC/USDT"):
    ...
    for i in range(30,len(df)):
        ...
        if position>0:
            ...
            if chg<=-sl_pct: balance+=position*price*(1-fee_pct/100); trades.append({"i":i,"type":"SELL","coin":coin,"entry_price":buy_price,"price":price,"pnl":round(chg,2),"reason":"Stop Loss","equity":balance}); position=0
            elif chg>=tp_pct: balance+=position*price*(1-fee_pct/100); trades.append({"i":i,"type":"SELL","coin":coin,"entry_price":buy_price,"price":price,"pnl":round(chg,2),"reason":"Take Profit","equity":balance}); position=0
        ...
        if res["signal"]=="BUY" and position==0 and balance>10:
            ...
            if cost<=balance: position=inv/price; buy_price=price; balance-=cost; trades.append({"i":i,"type":"BUY","coin":coin,"entry_price":price,"price":price,"pnl":0,"reason":res["note"],"equity":balance+position*price})
        elif res["signal"]=="SELL" and position>0:
            balance+=position*price*(1-fee_pct/100); pnl=(price-buy_price)/buy_price*100; trades.append({"i":i,"type":"SELL","coin":coin,"entry_price":buy_price,"price":price,"pnl":round(pnl,2),"reason":res["note"],"equity":balance}); position=0
    ...
    for e in equity:
        ...
        dd=(peak-e)/peak*100
        if dd>mdd: mdd=dd
    return {"final":round(final,2),"total_ret":round(total_ret,2),"win_rate":round(wr,1),"trades":len(sells),"avg_win":round(aw,2),"avg_loss":round(al2,2),"profit_factor":round(pf,2),"max_dd":round(mdd,2),"equity":equity,"trade_log":trades,"df":df,"strategy":strategy,"coin":coin,"capital":capital,"sharpe":round(total_ret/max(mdd,0.1)*0.45,2)}
...
def chart_sim_history():
    ...
    hover_labels=[f"{r.get('coin','?').replace('/USDT','')} · {r['strategy'][:18]}" for r in h]
    fig=go.Figure(go.Bar(x=list(range(len(h))),y=[r["return"] for r in h],marker_color=colors,text=[f"{r.get('coin','?').replace('/USDT','')} {r['return']:+.1f}%" for r in h],textposition="outside",textfont=dict(color="#334155",size=9),customdata=hover_labels,hovertemplate="%{customdata}: %{y:.2f}%<extra></extra>"))
...
# ─────────────────────────────────────────────────────────────────────────────
# PERSISTENT AUTO-RUN STATUS BAR (visible on all tabs)
# ─────────────────────────────────────────────────────────────────────────────
if st.session_state.auto_sim_running:
    _done = st.session_state.auto_sim_done
    _max  = st.session_state.auto_sim_max
    _pct  = int(_done / _max * 100) if _max > 0 else 0
    _label = f"{_done}/{_max} runs ({_pct}%)" if _max > 0 else f"{_done} runs — unlimited"
    _col1, _col2 = st.columns([5, 1])
    with _col1:
        st.markdown(f"""
        <div style='background:#eff6ff;border:2px solid #2563eb;border-radius:10px;
                    padding:12px 20px;display:flex;align-items:center;gap:16px;'>
          <div style='width:10px;height:10px;background:#2563eb;border-radius:50%;
                      animation:pulse 1s infinite;flex-shrink:0;'></div>
          <div>
            <span style='font-weight:700;color:#1d4ed8;font-size:0.9rem;'>⟳ AUTO-SIM RUNNING</span>
            <span style='color:#3b82f6;font-size:0.8rem;margin-left:12px;'>{_label}</span>
            <span style='color:#94a3b8;font-size:0.75rem;margin-left:12px;'>
              Strategy: {st.session_state.auto_sim_strat[:30]}
            </span>
          </div>
        </div>
        ...
        """, unsafe_allow_html=True)
    with _col2:
        if st.button("⏹ STOP AUTO-RUN", use_container_width=True, key="global_stop_btn", type="primary"):
            st.session_state.auto_sim_running = False
            st.rerun()
...
# ─────────────────────────────────────────────────────────────────────────────
# ─────────────────────────────────────────────────────────────────────────────
# CONTINUOUS SIM LOOP — runs at top level so st.rerun() works properly
# ─────────────────────────────────────────────────────────────────────────────
if st.session_state.auto_sim_running:
    _max  = st.session_state.auto_sim_max
    _done = st.session_state.auto_sim_done
    # Stop condition
    if _max > 0 and _done >= _max:
        st.session_state.auto_sim_running = False
    else:
        _strat = st.session_state.auto_sim_strat
        _coin  = st.session_state.auto_sim_coin
        _cur_p = st.session_state.strategy_params.get(_strat, STRATEGIES[_strat]["params"])
        _res   = simulate_strategy(
            _strat,
            st.session_state.auto_sim_cap,
            st.session_state.auto_sim_candles,
            st.session_state.auto_sim_pos,
            st.session_state.auto_sim_fee,
            st.session_state.auto_sim_sl,
            st.session_state.auto_sim_tp,
            _cur_p,
            coin=_coin,
        )
        st.session_state.sim_results   = _res
        st.session_state.sim_count    += 1
        st.session_state.auto_sim_done += 1
        auto_ai_tune(_strat, _res)
        st.session_state.sim_history.insert(0, {
            "coin":     _coin,
            "strategy": _strat,
            "return":   round(_res["total_ret"], 2),
            "win_rate": round(_res["win_rate"], 1),
            "trades":   _res["trades"],
            "max_dd":   round(_res["max_dd"], 1),
            "time":     datetime.now().strftime("%H:%M:%S"),
        })
        if len(st.session_state.sim_history) > 200:
            st.session_state.sim_history = st.session_state.sim_history[:200]
        for _t in _res["trade_log"]:
            if _t["type"] == "SELL":
                st.session_state.trade_journal.insert(0, {
                    "time":     datetime.now().strftime("%H:%M:%S"),
                    "source":   f"SIM#{st.session_state.sim_count}",
                    "coin":     _coin,
                    "strategy": _strat,
                    "side":     "SELL",
                    "entry_price": round(_t.get("entry_price", _t["price"]), 4),
                    "exit_price":  round(_t["price"], 4),
                    "pnl":      _t["pnl"],
                    "reason":   _t["reason"][:40],
                })
        if len(st.session_state.trade_journal) > 1000:
            st.session_state.trade_journal = st.session_state.trade_journal[:1000]
        push_alert(
            f"Auto #{st.session_state.sim_count} {_coin} [{_strat[:14]}]: "
            f"{_res['total_ret']:+.2f}% · WR {_res['win_rate']:.0f}%",
            "success" if _res["total_ret"] > 0 else "warning",
        )
        _delay = st.session_state.auto_sim_delay
        if _delay > 0:
            time.sleep(_delay)
        st.rerun()
...
# ══════════════════════════════════════════════════════════════════════════════
# TAB 4 — SIMULATION  (continuous auto-loop)
# ══════════════════════════════════════════════════════════════════════════════
with tab_sim:
    ...
    # Running banner
    if st.session_state.auto_sim_running:
        done_  = st.session_state.auto_sim_done
        max_   = st.session_state.auto_sim_max
        pct_   = int((done_ / max_ * 100)) if max_ > 0 else 0
        label_ = f"Run {done_}/{max_} ({pct_}%)" if max_ > 0 else f"Run {done_} — unlimited"
        st.markdown(f"""
        <div style='background:#eff6ff;border:2px solid #2563eb;border-radius:10px;
                    padding:14px 20px;margin-bottom:16px;display:flex;
                    justify-content:space-between;align-items:center;'>
          <div>
            <span style='font-size:1rem;font-weight:700;color:#1d4ed8;'>⟳ AUTO-RUNNING</span>
            <span style='font-size:0.8rem;color:#3b82f6;margin-left:12px;'>{label_}</span>
          </div>
          <span style='font-size:0.75rem;color:#64748b;'>{st.session_state.auto_sim_coin} · {st.session_state.auto_sim_strat[:22]}</span>
        </div>
        """, unsafe_allow_html=True)
    ...
    with sc1:
        ...
        running = st.session_state.auto_sim_running

        # Disable inputs while running
        sim_strat   = st.selectbox("Strategy", list(STRATEGIES.keys()),
                                   index=list(STRATEGIES.keys()).index(st.session_state.auto_sim_strat),
                                   disabled=running)
        sim_coin    = st.selectbox("Asset", COINS,
                                   index=COINS.index(st.session_state.auto_sim_coin) if st.session_state.auto_sim_coin in COINS else 0,
                                   format_func=lambda x: x.replace("/USDT",""),
                                   disabled=running)
        sim_cap     = st.number_input("Capital (USDT)", 100, 1_000_000,
                                      st.session_state.auto_sim_cap, 500, disabled=running)
        sim_candles = st.slider("Candles", 50, 600,
                                st.session_state.auto_sim_candles, 25, disabled=running)
        sim_pos     = st.slider("Position Size %", 10, 100,
                                st.session_state.auto_sim_pos, 5, disabled=running)
        sim_fee     = st.number_input("Fee %", 0.0, 1.0,
                                      st.session_state.auto_sim_fee, 0.01, format="%.2f", disabled=running)
        sim_sl      = st.number_input("Stop Loss %", 0.5, 20.0,
                                      st.session_state.auto_sim_sl, 0.5, format="%.1f", disabled=running)
        sim_tp      = st.number_input("Take Profit %", 1.0, 50.0,
                                      st.session_state.auto_sim_tp, 0.5, format="%.1f", disabled=running)

        # Continuous settings
        st.markdown("---")
        st.markdown("**Auto-Run Settings**")
        sim_delay = st.slider("Delay between runs (s)", 0, 10, st.session_state.auto_sim_delay, 1, disabled=running)
        sim_max   = st.number_input("Max runs (0 = unlimited)", 0, 10000,
                                    st.session_state.auto_sim_max, 10, disabled=running)

        # Save params to session
        st.session_state.auto_sim_strat   = sim_strat
        st.session_state.auto_sim_coin    = sim_coin
        st.session_state.auto_sim_cap     = sim_cap
        st.session_state.auto_sim_candles = sim_candles
        st.session_state.auto_sim_pos     = sim_pos
        st.session_state.auto_sim_fee     = sim_fee
        st.session_state.auto_sim_sl      = sim_sl
        st.session_state.auto_sim_tp      = sim_tp
        st.session_state.auto_sim_delay   = sim_delay
        st.session_state.auto_sim_max     = sim_max

        st.markdown("")
        b1, b2 = st.columns(2)
        with b1:
            if not running:
                if st.button("▶ RUN ONCE", use_container_width=True, key="btn_run_once"):
                    cur_p = st.session_state.strategy_params.get(sim_strat, STRATEGIES[sim_strat]["params"])
                    res   = simulate_strategy(sim_strat,sim_cap,sim_candles,sim_pos,sim_fee,sim_sl,sim_tp,cur_p,coin=sim_coin)
                    st.session_state.sim_results  = res
                    st.session_state.sim_count   += 1
                    st.session_state.auto_sim_done += 1
                    auto_ai_tune(sim_strat, res)
                    st.session_state.sim_history.insert(0,{"coin":sim_coin,"strategy":sim_strat,"return":round(res["total_ret"],2),"win_rate":round(res["win_rate"],1),"trades":res["trades"],"max_dd":round(res["max_dd"],1),"time":datetime.now().strftime("%H:%M:%S")})
                    for t in res["trade_log"]:
                        if t["type"]=="SELL":
                            st.session_state.trade_journal.insert(0,{"time":datetime.now().strftime("%H:%M:%S"),"source":f"SIM#{st.session_state.sim_count}","coin":sim_coin,"strategy":sim_strat,"side":"SELL","entry_price":round(t.get("entry_price",t["price"]),4),"exit_price":round(t["price"],4),"pnl":t["pnl"],"reason":t["reason"][:40]})
                    push_alert(f"Sim #{st.session_state.sim_count} {sim_coin} [{sim_strat[:14]}]: {res['total_ret']:+.2f}% · WR {res['win_rate']:.0f}%","success" if res["total_ret"]>0 else "warning")
                    st.rerun()
            else:
        ...
        with b2:
            if not running:
                if st.button("⟳ AUTO-RUN", use_container_width=True, key="btn_auto_run"):
                    st.session_state.auto_sim_done    = 0
                    st.session_state.auto_sim_running = True
                    st.rerun()
            else:
                st.markdown(f"""
                <div style='background:#eff6ff;border:1px solid #bfdbfe;border-radius:8px;
                            padding:10px;text-align:center;font-size:0.78rem;color:#1d4ed8;font-weight:600;'>
                  ⟳ RUNNING<br>
                  <span style='font-size:0.65rem;font-weight:400;color:#3b82f6;'>
                    {st.session_state.auto_sim_done} run(s) done
                  </span>
                </div>
                """, unsafe_allow_html=True)
        ...
        if t_runs > 0:
            ...
            wr_c    = "#059669" if avg_wr>=55 else ("#d97706" if avg_wr>=45 else "#dc2626")
            ret_c   = "#059669" if avg_ret>=0 else "#dc2626"
            wrs     = tracker["win_rates"]
            arr     = "↑" if len(wrs)>=2 and wrs[-1]>wrs[0] else ("↓" if len(wrs)>=2 and wrs[-1]<wrs[0] else "→")
            mode    = "30/70 rolling" if t_runs>=4 else ("60/40 blend" if t_runs>=2 else "single run")
            st.markdown(f"""
            <div style='background:#f8fafc;border:1px solid #e2e8f0;border-radius:8px;
                        padding:14px 16px;margin-top:12px;'>
              <div style='font-size:0.68rem;color:#2563eb;letter-spacing:1px;
                          margin-bottom:10px;font-weight:600;'>🤖 AI TUNING STATUS</div>
              <div style='display:grid;grid-template-columns:1fr 1fr;gap:6px;font-size:0.75rem;color:#334155;'>
                <div>Runs <strong style='color:#7c3aed;'>{t_runs}</strong></div>
                <div>Done <strong style='color:#334155;'>{st.session_state.auto_sim_done}</strong></div>
                <div>Avg WR <strong style='color:{wr_c};'>{avg_wr:.0f}%</strong> {arr}</div>
                <div>Avg Ret <strong style='color:{ret_c};'>{avg_ret:+.1f}%</strong></div>
                <div>Avg DD <span style='color:#d97706;'>{avg_dd:.1f}%</span></div>
                <div style='color:#2563eb;font-size:0.65rem;'>{mode}</div>
              </div>
              <div style='margin-top:10px;height:4px;background:#e2e8f0;border-radius:2px;'>
                <div style='height:100%;width:{min(t_runs*10,100)}%;
                            background:linear-gradient(90deg,#2563eb,#7c3aed);
                            border-radius:2px;transition:width 0.3s;'></div>
              </div>
              <div style='font-size:0.62rem;color:#94a3b8;margin-top:3px;'>
                Confidence {min(t_runs*10,100)}% · {t_runs}/10 runs · updated {last_upd}
              </div>
            </div>
            """, unsafe_allow_html=True)
            if len(wrs) >= 2:
                tc = "#059669" if arr=="↑" else ("#dc2626" if arr=="↓" else "#d97706")
                st.markdown(f"<div style='font-size:0.68rem;color:#64748b;margin-top:6px;'>WR trend: <span style='color:{tc};font-weight:600;'>{' → '.join([f'{w:.0f}%' for w in wrs[-5:]])} {arr}</span></div>", unsafe_allow_html=True)
        else:
            st.markdown("""
            <div style='background:#f8fafc;border:1px solid #e2e8f0;border-radius:8px;
                        padding:12px 14px;margin-top:12px;font-size:0.75rem;color:#94a3b8;'>
              🤖 AI auto-tuning starts after first run
            </div>
            """, unsafe_allow_html=True)
    ...
    with sc2:
        if st.session_state.sim_results:
            ...
            _rc = res.get("coin","BTC/USDT"); _rs = res.get("strategy","—")
            st.markdown(f"<div style='background:#f8fafc;border:1px solid #e2e8f0;border-radius:8px;padding:10px 16px;margin-bottom:12px;display:flex;gap:16px;align-items:center;'><span style='font-size:1rem;font-weight:700;color:#0f172a;'>{_rc.replace('/USDT','')}</span><span style='color:#94a3b8;font-size:0.75rem;'>{_rc}</span><span style='background:#eff6ff;color:#2563eb;padding:2px 10px;border-radius:99px;font-size:0.72rem;font-weight:600;'>{_rs[:30]}</span></div>",unsafe_allow_html=True)
            ...
            with m7: st.metric("Prof Fac",  f"{res['profit_factor']:.2f}")
            ...
            <div style='text-align:center;padding:80px 0;'>
              <div style='font-size:4rem;color:#e2e8f0;'>◈</div>
              <div style='color:#94a3b8;font-size:0.9rem;margin-top:8px;'>
                Click ▶ RUN ONCE for a single run<br>
                Click ⟳ AUTO-RUN to loop continuously
              </div>
            </div>
            """, unsafe_allow_html=True)

    # ── Bottom section — performance + history ────────────────────────────────
    if st.session_state.sim_results:
        res = st.session_state.sim_results
        st.markdown("---")
        ...
        with af1:
            ...
            else:                 st.warning(f"⚠️ Under: {ret:.1f}% — AI adjusting params")
        ...
        with af2:
            ...
            if strat_log:
                for e in strat_log:
                    ...
                    rb = f" <span style='color:#94a3b8;font-size:0.6rem;'>[run {e.get('runs','?')} · WR {e.get('avg_wr','?')}%]</span>" if "runs" in e else ""
                    st.markdown(f"<div style='background:#f8fafc;border-left:3px solid #2563eb;padding:7px 11px;margin-bottom:4px;border-radius:0 6px 6px 0;font-size:0.73rem;color:#334155;'>{ic} {e['msg']}{rb}</div>", unsafe_allow_html=True)
            else:
                st.caption("AI log will appear here after first run")
        ...
        # Trade log
        st.markdown("---")
        st.markdown("#### TRADE LOG (latest sim)")
        if res["trade_log"]:
            _sim_coin = res.get("coin","BTC/USDT")
            tdf = pd.DataFrame([{
                "Asset":  t.get("coin",_sim_coin).replace("/USDT",""),
                "Type":   t["type"],
                "Price":  fmt_price(float(t["price"])),
                "P&L %":  (f"{t['pnl']:+.2f}%" if t["type"]=="SELL" else "—"),
                "Reason": t["reason"][:35],
                "Candle": f"#{t['i']}",
                "Equity": f"${t['equity']:,.2f}",
            } for t in reversed(res["trade_log"])])
            def st_s(v): return "color:#059669;font-weight:600" if v=="BUY" else ("color:#dc2626;font-weight:600" if v=="SELL" else "")
            def st_p(v):
                try: n=float(v.replace("%","").replace("+","")); return f"color:{'#059669' if n>=0 else '#dc2626'}"
                except: return "color:#64748b"
            st.dataframe(
                tdf.style.map(st_s, subset=["Type"]).map(st_p, subset=["P&L %"])
                   .set_properties(**{"font-family":"Inter,sans-serif","font-size":"12px"}),
                use_container_width=True, height=280,
            )
            # Quick BUY/SELL summary
            buys_  = [t for t in res["trade_log"] if t["type"]=="BUY"]
            sells_ = [t for t in res["trade_log"] if t["type"]=="SELL"]
            wins_  = [t for t in sells_ if t["pnl"]>0]
            st.markdown(f"<div style='display:flex;gap:12px;flex-wrap:wrap;margin-top:6px;font-size:0.75rem;'>"
                        f"<span style='background:#ecfdf5;color:#065f46;padding:3px 10px;border-radius:99px;'>🟢 {len(buys_)} BUY trades on {_sim_coin}</span>"
                        f"<span style='background:#fef2f2;color:#7f1d1d;padding:3px 10px;border-radius:99px;'>🔴 {len(sells_)} SELL trades</span>"
                        f"<span style='background:#eff6ff;color:#1e40af;padding:3px 10px;border-radius:99px;'>✅ {len(wins_)} winners ({len(wins_)/max(len(sells_),1)*100:.0f}%)</span>"
                        f"</div>",unsafe_allow_html=True)
            _tlog_csv = tdf.to_csv(index=False).encode("utf-8")
            st.download_button("⬇ Download trade log (.csv)", data=_tlog_csv,
                               file_name=f"nexus_trades_sim{st.session_state.sim_count}.csv",
                               mime="text/csv", key=f"tlog_dl_{st.session_state.sim_count}")
...
with tab_journal:
    ...
    with jc1:
        if st.session_state.trade_journal:
            ...
            show_cols=[c for c in ["time","coin","strategy","side","entry_price","exit_price","pnl","reason","source"] if c in jdf.columns]
